...
package vendor.qti.microdroid_test.demo;
...
import vendor.qti.microdroid_test.ITestPayload;
...
public class MainActivity extends AppCompatActivity {
    private static final String TAG = "TestPayloadAPK";
    ...
    public static class VirtualMachineModel extends AndroidViewModel {
        private static final String VM_NAME = "test_payload_vm";
        ...
        public void run(boolean debug) {
            ...
            VirtualMachineCallback callback =
                    new VirtualMachineCallback() {
                        ...
                        public void onPayloadReady(VirtualMachine vm) {
                            ...
                            mService.execute(() -> testVmAddInteger(vm));
                            mService.execute(() -> testVmHello(vm));
                        }

                        private void testVmAddInteger(VirtualMachine vm) {
                            IBinder binder;
                            try {
                                binder = vm.connectToVsockServer(ITestPayload.SERVICE_PORT);
                            } catch (Exception e) {
                            ...
                            try {
                                ITestPayload testPayload = ITestPayload.Stub.asInterface(binder);
                                // int val1 = a;
                                // int val2 = b;
                                int ret = testPayload.addInteger(123,456);
                                mPayloadOutput.postValue(
                                        String.format(
                                                "(VM payload AddInteger: %d + %d = %d)",
                                                123, 456, ret));
                        ...
                        private void testVmHello(VirtualMachine vm) {
                            IBinder binder;
                            try {
                                binder = vm.connectToVsockServer(ITestPayload.SERVICE_PORT);
                            } catch (Exception e) {
                            ...
                            try {
                                ...
                                // String buffer = input_name;
                                String msg = testPayload.hello("TestPayloadAPK");
                                mPayloadOutput.postValue(
                                    String.format(
                                            "(VM payload Hello: %s)",
                                            msg));
            ...
            try {
                ...
                Log.i(TAG, "setting payload binary name");
                builder.setPayloadBinaryName("libtest_payload.so");
                ...
                Log.i(TAG, "The VM Capabilities => " + capabilities);
