...
#include <aidl/android/system/virtualizationcommon/IGuestAgent.h>
...
#include <aidl/vendor/qti/microdroid_test/ITestPayload.h>
...
using aidl::vendor::qti::microdroid_test::ITestPayload;
...
Result<std::shared_ptr<IVirtualizationService>> connect_service(int fd) {
    ...
    session->setMaxIncomingThreads(VIRTMGR_THREADS);
    session->setMaxOutgoingConnections(VIRTMGR_THREADS);

    unique_fd bootstrapFd(dup(fd));
    if (!bootstrapFd.ok()) {
        return Error() << "Invalid bootstrap fd " << fd;
    }

    status_t status = session->setupUnixDomainSocketBootstrapClient(std::move(bootstrapFd));
    if (status != OK) {
        return Error() << "Failed to set up Unix Domain RPC client with bootstrap fd "
                       << fd << " error: " << statusToString(status);
    }

    AIBinder* binder = AIBinder_fromPlatformBinder(session->getRootObject());
...
public:
    ...
    Result<void> wait_for_state(VirtualMachineState state) {
        std::unique_lock lock(mMutex);
        mCv.wait_for(lock, 20s, [this, &state] {
...
// Result<std::shared_ptr<ITestPayload>> connect_to_vm_payload(std::shared_ptr<IVirtualMachine> vm) {
Result<std::shared_ptr<ITestPayload>> connect_to_vm_payload(std::shared_ptr<IVirtualMachine> vm) {
    sp<RpcSession> session = RpcSession::make();
    session->setMaxIncomingThreads(1);

    auto requestFd = [vm]() -> unique_fd {
        ScopedFileDescriptor sock_fd;
        ScopedAStatus ret = vm->connectVsock(ITestPayload::SERVICE_PORT, &sock_fd);
        if (!ret.isOk()) {
            return unique_fd{};
        }
        return unique_fd{sock_fd.release()};
    };

    status_t status = session->setupPreconnectedClient(unique_fd{}, requestFd);
    if (status != OK) {
        return Error() << "Failed to set up preconnected client: " << statusToString(status);
    }

    AIBinder* binder = AIBinder_fromPlatformBinder(session->getRootObject());
    return ITestPayload::fromBinder(SpAIBinder{binder});
}


// Do something with the service in the VM
Result<void> do_something(ITestPayload& payload) {
    int32_t result;
...
Result<void> inner_main() {
    std::string work_dir_path("/data/local/tmp/microdroid_native_test_cpp_workdir/");
    ...
    VirtualMachineAppConfig app_config = OR_RETURN(
            create_vm_config(*service, work_dir_path, "my_vm",
                             "/data/local/tmp/app/EmptyTestPayloadApp/EmptyTestPayloadApp.apk", "libtest_payload.so",
    ...
    std::shared_ptr<ITestPayload> payload = OR_RETURN(connect_to_vm_payload(vm));
